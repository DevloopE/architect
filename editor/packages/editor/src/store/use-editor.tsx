  ...
  persist(
    (set, get) => ({
      ...
      setPhase: (phase) => {
        ...
        // Helper to find building and ground level (level 1)
        const selectBuildingAndLevel0 = () => {
          ...
          // If no level selected, find ground level (level 1) in the building
          if (buildingId && !viewer.selection.levelId) {
            const buildingNode = scene.nodes[buildingId] as BuildingNode
            const groundLevelId = buildingNode.children.find((childId) => {
              const levelNode = scene.nodes[childId] as LevelNode
              return levelNode?.type === 'level' && levelNode.level === 1
            })
            if (groundLevelId) {
              viewer.setSelection({ levelId: groundLevelId as LevelNode['id'] })
            } else if (buildingNode.children[0]) {
              // Fallback to first level if ground level doesn't exist
