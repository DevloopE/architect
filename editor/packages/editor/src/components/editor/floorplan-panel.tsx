...
export function FloorplanPanel() {
  ...
  useEffect(() => {
    if (!(phase === 'site' && levelNode?.type === 'level' && levelNode.level !== 1)) {
      return
    }
  ...
  const isGroundFloorSelected = levelNode?.type === 'level' && levelNode.level === 1
  ...
  const restoreGroundLevelStructureSelection = useCallback(() => {
    ...
    const nextGroundLevelId =
      nextBuildingId && nextBuildingId === currentBuildingId
        ? (floorplanLevels.find((level) => level.level === 1)?.id ??
        ...
        : (() => {
            ...
            return (
              buildingLevels.find((level) => level.level === 1)?.id ?? buildingLevels[0]?.id ?? null
            )
          })()
  ...
  const handleSiteEditShortcutSelect = useCallback(() => {
    if (!(levelNode?.type === 'level' && levelNode.level === 1)) {
      return
    }
